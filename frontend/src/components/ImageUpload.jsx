                ...
                productData.productImg.map((file, idx) => {
                    ...
                    return (
                        <Card key={idx} className='relative group overflow-hidden'>
                            <CardContent>
                                <img src={preview} alt={`Preview ${idx}`} width={200} height={200} className='object-cover h-32 w-full rounded-md' />
                                {/* remove button */}
                                <button onClick={() => removeImage(idx)} className='absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600'>
                                    <X size={14}/>
                                </button>
                            </CardContent>
                        </Card>
