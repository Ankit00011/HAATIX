...
export const verify = async(req, res)=>{
    try {
        ...
        const token = authHeader.split(" ")[1] //Bearer jnjnvkvtree
        let decoded
        try {
...
export const login = async(req, res)=>{
    try {
        ...
        // genertate token
        const accessToken = jwt.sign({id:existingUser}, process.env.SECRET_KEY, {expiresIn: '10d'})
        const refreshToken = jwt.sign({id:existingUser}, process.env.SECRET_KEY, {expiresIn: '30d'})  
...
export const changePassword = async(req, res)=>{
    try {
        ...
        return res.status(200).json({
            success: true,
            message: "Password chanegd successfully"
        })
    } catch (error) {
        return res.status(500).json({
            success: false,
